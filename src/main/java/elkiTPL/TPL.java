...
public class TPL<O extends DoubleVector, D extends DoubleDistance> extends TPLRkNNQuery<O, DoubleDistance> {
  ...
  public TPL(RStarTree tree, DistanceQuery<O, DoubleDistance> distancequery, boolean withClipping){
    super(distancequery);
    this.tree = tree;
    this.withClipping = withClipping;
    min_card = (int) (0.4 * tree.getRoot().getCapacity());
  }
  
  
  public PriorityQueue<SimpleEntry<Double, TPLEntry>> initializeAPL(){
  ...
  public ArrayList<ArrayList<?>> filter(O q, int k){
    ...
    while(!apl.isEmpty()) {
      ...
      SpatialEntry se = entry.getValue().getEntry();
      if (se.isLeafEntry()){
      ...
      if ( val == Double.POSITIVE_INFINITY ) {
        ...
        if(se.isLeafEntry()) {
          ...
          if (node.isLeaf()){
            ...
            for (SpatialEntry N_i : node.getEntries()) {
    ...
    System.out.println("    Size of candidate Set: "+candidateSet.size()+", "+candidateSet);
    System.out.println("    Size of refinement Set: "+refinementSet.size());
    return res;
  }
  
  
  
  public DistanceDBIDList<DoubleDistance> refine(O q, int k, ArrayList<SpatialPointLeafEntry> candidateSet, ArrayList<TPLEntry> refinementSet){
    ...
    System.out.println("  Size of refinementSetPoints: "+refinementSetPoints.size());
    System.out.println("  Size of refinementSetNodes: "+refinementSetNodes.size());
    HashMap<DBID, HashMap<Integer, TPLEntry>> toVisits = new HashMap<DBID, HashMap<Integer, TPLEntry>>();
    
    if (!withClipping){
      while (candidateSet.size() > k){
        SpatialPointLeafEntry max = null;
        max = candidateSet.get(0);
        for (int j = 0; j < candidateSet.size(); j++){
          if (dist.doubleMinDist(max, q) < dist.doubleMinDist(candidateSet.get(j), q)){
            max = candidateSet.get(j);
          }
        }
        
        candidateSet.remove(max);
      }
    }
    ...
    // repeat
    while(true){
      k_refinement_round(q, k, candidateSet, refinementSetPoints, refinementSetNodes, toVisits, result);
      ...
      // and access bestNode
      RStarTreeNode node = tree.getNode(bestNode.getEntry());
      
      for(SpatialEntry e : node.getEntries()) {
    ...
    System.out.println("  Size of candidateSet after refinement: "+candidateSet.size());
  ...
  public void k_refinement_round(O q, int k, ArrayList<SpatialPointLeafEntry> candidateSet,
      ArrayList<SpatialPointLeafEntry> refinementSetPoints, ArrayList<TPLEntry> refinementSetNodes,
      HashMap<DBID, HashMap<Integer, TPLEntry>> toVisits, GenericDistanceDBIDList<DoubleDistance> result) {
    ...
    nextCandidate: for (int i = 0; i < candidateSet.size(); i++){
      ...
      if (toVisits.get(p.getDBID()) != null && toVisits.get(p.getDBID()).size() == 0){
        ...
        DBID dbid = p.getDBID();
        DoubleDistance distance = (DoubleDistance) distanceQuery.distance(dbid, q);
  ...
  public double prune (NumberVector<?> q, int k, ArrayList<SpatialPointLeafEntry> candidateSet, SpatialEntry entry){
    int pruneCount = 0;
  ...
  public double k_trim (DoubleVector q, int k, ArrayList<SpatialPointLeafEntry> candidateSet, SpatialEntry entry){
    ...
    if (candidateSet.size() >= k){
      ...
      for (int i = 0; i < subsets.size(); i++){
        ...
        for (int j = 0; j < Nres.getDimensionality(); j++){
          if (Nres.getMin(j) != Nres.getMax(j)){
            empty = false;
          }
        }
        if (empty){
  ...
  public static SpatialComparable clipping(DoubleVector q, ArrayList<SpatialPointLeafEntry> subset, SpatialComparable rect){
  ...
  @Override
  public DistanceDBIDList<DoubleDistance> getRKNNForObject(O q, int k) {
    ...
    if (filtered.get(0).get(0) instanceof SpatialPointLeafEntry){
      candidateSet = (ArrayList<SpatialPointLeafEntry>) filtered.get(0);
      refinementSet = (ArrayList<TPLEntry>) filtered.get(1);
    }
    else {
      candidateSet = (ArrayList<SpatialPointLeafEntry>) filtered.get(1);
      refinementSet = (ArrayList<TPLEntry>) filtered.get(0);
    }
  ...
  @Override
  public List<? extends DistanceDBIDList<DoubleDistance>> getRKNNForBulkDBIDs(ArrayDBIDs ids, int k) {
    // TODO Auto-generated method stub
    List<DistanceDBIDList<DoubleDistance>> result = new ArrayList<DistanceDBIDList<DoubleDistance>>();
  ...
  @Override
  public DistanceDBIDList<DoubleDistance> getRKNNForDBID(DBIDRef id, int k) {
    // TODO Auto-generated method stub
