...
public class RnnTableWindow extends JFrame implements ActionListener, ResultVisualizer {
	...
	public RnnTableWindow(ProbabilisticGraph graph, int start, boolean useRelative) {
		...
		title = "Reverse Nearest Neighbors from Node \"" + start + "\"";
		setTitle(title);
		createContent();
...
//							greenRows.clear();
							...
							for (Iterator<Integer> iter = sortedData.keySet().iterator(); iter.hasNext();) {
								...
								System.out.println("Node-ID= " + key + "\tResult= " + ergebnis + 
	...
	protected Vector<Integer> getValue() {
		return GraphAlgorithm.getReverseNearestNeighbors(start);
	}
