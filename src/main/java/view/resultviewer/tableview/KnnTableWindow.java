...
public class KnnTableWindow extends AbstractTableWindow {
	
	public KnnTableWindow(int start, int k, boolean useRelative) {
		super(start, k, useRelative);
	}
	...
	protected void setGlobalTitle() {
		this.setTitle(k + "-Nearest Neighbors from Node \"" + start + "\"");
	}
		...
		worker = new SwingWorker<Object, Void>() {
			...
			public Object doInBackground() {
				while(true && !isCancelled()) {
					Vector<Integer> val = GraphAlgorithm.getKNearestNeighboursOnTheFly(start, k);
					for(Integer v : val) {
